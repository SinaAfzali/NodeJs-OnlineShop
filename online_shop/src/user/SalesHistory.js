...
const SalesHistory = ({filter}) => {
  ...
  const handleViewMore = (sale) => {
        ...
        if(moreBtn.innerHTML === 'جزئیات بیشتر'){
          moreDiv.innerHTML = `<p>محصولات فروخته شده : </p><p></p>`;
        ...
        {salesData.map((sale) => (
          <div key={sale.id} className="sale-item">
            <div className="sale-info">
              ...
              <p className="date">خریدار: {sale.customer}</p>
              <p id='receipt-number' className="receipt-number">کدپیگیری : {sale.receiptNumber}</p>
              <p className="count">تعداد محصولات: {sale.count}</p>
            </div>
            <div className='more-datails-div' id={`more-details${sale.id}`}><p>محصولات فروخته شده : </p><p></p></div>
